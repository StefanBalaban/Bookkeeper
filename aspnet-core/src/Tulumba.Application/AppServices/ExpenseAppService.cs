...
namespace Tulumba.Application.AppServices
{
    ...
    {
        private IRepository<Expense, Guid> _expenseRepository;
        private IRepository<ExpenseType, Guid> _expenseTypeRepository;
        ...
        public override async Task<ExpenseDto> UpdateAsync(Guid id, CreateUpdateExpenseDto input)
        {
            ...
            {
                var dailyExpenseType = (await _expenseTypeRepository.FindAsync(entity.ExpenseTypeId)).ExpensePeriod == ExpensePeriod.Dnevni ? true : false;
        ...
        [Authorize(Permissions.TulumbaPermissions.Expenses.Get)]
        public override async Task<PagedResultDto<ExpenseDto>> GetListAsync(GetExpenseListDto input)
        {
                ...
                )
            {
                ...
                {
                    input.Sorting = nameof(Expense.Date);
                }
            ...
            {
                expenses = (await _expenseRepository.GetListAsync(x =>
                    ...
                    (!input.ExpenseCategory.HasValue || x.ExpenseType.ExpenseCategory == input.ExpenseCategory))).Skip(input.SkipCount).Take(input.MaxResultCount).ToList();
            }
